...
Template.gameView.helpers({
  ...
  gameFinished: function () {
    var game = getCurrentGame();
    var timeRemaining = getTimeRemaining(game);
  ...
  timeRemaining: function () {
    ...
    if(game.currentMode === 0){
      result = "Game starting in: " + moment(timeRemaining).format('mm[<span>:</span>]ss');;
    }
    //10 second transition to night time
    else if(game.currentMode === 1){
      result = "Transition to night: " + moment(timeRemaining).format('mm[<span>:</span>]ss');;
    }
    //30 second mafia turn
    else if(game.currentMode === 2){
      result = "Mafia take action now: " + moment(timeRemaining).format('mm[<span>:</span>]ss');;
    }
    //10 second transition
    else if(game.currentMode === 3){
      result = "Sleep time: " + moment(timeRemaining).format('mm[<span>:</span>]ss');;
    }
    //30 second nurse turn
    else if(game.currentMode === 4){
      result = "Nurse take action now: " + moment(timeRemaining).format('mm[<span>:</span>]ss');;
    }
    //10 second transition
    else if(game.currentMode === 5){
      result = "Sleep time: " + moment(timeRemaining).format('mm[<span>:</span>]ss');;
    }
    //30 second cop turn
    else if(game.currentMode === 6){
      result = "Cop take action now: " + moment(timeRemaining).format('mm[<span>:</span>]ss');;
    }
    //10 second transition
    else if(game.currentMode === 7){
      result = "Sleep time: " + moment(timeRemaining).format('mm[<span>:</span>]ss');;
    }
    //30 second cop turn
    else if(game.currentMode === 8){
      result = "Day Time!: " + moment(timeRemaining).format('mm[<span>:</span>]ss');;
    }
    
    if(timeRemaining <= 0){
      Games.update(game.id, {$set: {currentMode: game.currentMode+1}});
      game.currentMode = game.currentMode+1;
      if(game.currentMode===1 || game.currentMode===3 || game.currentMode===5 || game.currentMode===7){
        Games.update(game.id, {$set: {endTime: moment().add(13, 'seconds').valueOf()}});
      }
      else if(game.currentMode===0 || game.currentMode===2 || game.currentMode===4 || game.currentMode===6){
        Games.update(game.id, {$set: {endTime: moment().add(33, 'seconds').valueOf()}});
      }
      else if(game.currentMode===8){
        Games.update(game.id, {$set: {endTime: moment().add(93, 'seconds').valueOf()}});
      }
      else if(game.currentMode===9){
        Games.update(game.id, {$set: {endTime: moment().add(13, 'seconds').valueOf()}});
        Games.update(game.id, {$set: {currentMode: 1}});
      }
    }
    
    return result;//moment(timeRemaining).format('mm[<span>:</span>]ss');
  }
});
